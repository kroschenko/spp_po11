...
class PythagorasTreeWidget(QWidget):
    ...
    def paintEvent(self, _):
        ...
        self.draw_tree(painter, start_point, length, -math.pi / 2, self.depth)
    ...
    def draw_tree(self, painter, start_point, length, angle, depth):
        ...
        self.draw_branch(painter, start_point, end_point, depth, color)

        if depth > 1:
            new_length = length * self.ratio
            self.draw_tree(painter, end_point, new_length, angle - self.angle, depth - 1)
            self.draw_tree(painter, end_point, new_length, angle + self.angle, depth - 1)
    ...
    @staticmethod
    def draw_branch(painter, start_point, end_point, depth, color):
        pen = QPen(color)
