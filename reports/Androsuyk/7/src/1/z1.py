...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Label(frame, text="Треугольник (3 точки):").grid(
        ...
        tk.Button(frame, text="Создать треугольник",
                  command=self.create_triangle).grid(
                      row=3, column=0, columnspan=6, pady=5)

        tk.Label(frame, text="Точки (x,y через запятую, по одной в строке):").grid(
        ...
        tk.Button(frame, text="Добавить точки",
                  command=self.add_points).grid(
                      row=6, column=0, columnspan=6, pady=5)

        tk.Label(frame, text="Скорость анимации (сек):").grid(
        ...
        tk.Button(frame, text="Старт/Пауза",
                  command=self.toggle_animation).grid(row=7, column=4)
        tk.Button(frame, text="Скриншот",
                  command=self.take_screenshot).grid(row=7, column=5)

        self.canvas = tk.Canvas(self, bg="white", width=680, height=400)
        self.canvas.pack(pady=10)

        self.status_label = tk.Label(
            self, text="Статус: Ожидание треугольника и точек")
    ...
    def create_triangle(self):
        try:
            ...
            self.triangle = Triangle(coords[0], coords[1], coords[2])
            self.status_label.config(text="Треугольник создан")
            self.draw()
        except ValueError:
            messagebox.showerror(
                "Ошибка", "Введите корректные числа для треугольника")
    ...
    def add_points(self):
        ...
        for line in lines:
            ...
            try:
                ...
                messagebox.showerror(
                    "Ошибка", f"Неверный формат точки: {line}\n{str(e)}")
                return
    ...
    def draw(self):
        ...
        margin = 20
        all_x = [self.triangle.p1.x, self.triangle.p2.x, self.triangle.p3.x] + [
            p.x for p in self.points]
        all_y = [self.triangle.p1.y, self.triangle.p2.y, self.triangle.p3.y] + [
    ...
    def toggle_animation(self):
        if not self.triangle or not self.points:
            messagebox.showwarning(
                "Внимание", "Сначала создайте треугольник и добавьте точки")
            return
        ...
        if self.is_running:
            ...
            try:
                ...
                messagebox.showerror(
                    "Ошибка", "Скорость должна быть положительным числом")
                return
    ...
    def animate_points(self):
        ...
        margin = 20
        all_x = [self.triangle.p1.x, self.triangle.p2.x, self.triangle.p3.x] + [
            pt.x for pt in self.points]
        all_y = [self.triangle.p1.y, self.triangle.p2.y, self.triangle.p3.y] + [
            pt.y for pt in self.points]
        min_x, max_x = min(all_x), max(all_x)
        min_y, max_y = min(all_y), max(all_y)
        ...
        self.canvas.create_oval(
            cx-10, cy-10, cx+10, cy+10, outline="blue", width=3, tags="highlight")
