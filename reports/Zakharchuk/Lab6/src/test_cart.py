...
def test_negative_price_raises_error(empty_cart):  # pylint: disable=redefined-outer-name
    with pytest.raises(ValueError, match="Price cannot be negative"):
        empty_cart.add_item("Apple", -10.0)


def test_total_calculation(empty_cart):  # pylint: disable=redefined-outer-name
    empty_cart.add_item("Apple", 10.0)
...
def test_apply_discount(empty_cart, discount, expected_total):  # pylint: disable=redefined-outer-name
    empty_cart.add_item("Apple", 10.0)
...
def test_invalid_discount_raises_error(empty_cart, invalid_discount):  # pylint: disable=redefined-outer-name
    empty_cart.add_item("Apple", 10.0)
...
def test_log_purchase(mock_post, empty_cart):  # pylint: disable=redefined-outer-name
...
def test_apply_coupon_valid(empty_cart, coupon_code, expected_total, monkeypatch):  # pylint: disable=redefined-outer-name
...
def test_apply_coupon_invalid(empty_cart, monkeypatch):  # pylint: disable=redefined-outer-name
    coupons = {"SAVE10": 10, "HALF": 50}
    monkeypatch.setattr("shopping.coupons", coupons)
    empty_cart.add_item("Apple", 10.0)
