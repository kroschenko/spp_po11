...
class UserAccount:
    ...
    def add_purchase(self, title, cost):
        """Добавляет покупку книги и начисляет очки активности."""
        activity_points = int(cost)  # Очки активности равны стоимости книги
        self.user_activity += activity_points
        self.purchase_history.append((title, cost))
        print(f"Книга '{title}' стоимостью {cost} добавлена в историю покупок.")
    ...
    def show_purchase_history(self):
        ...
        print("\nИстория покупок:")
        for i, (item_title, item_cost) in enumerate(self.purchase_history, start=1):
            print(f"{i}. Книга: {item_title}, Стоимость: {item_cost}")
