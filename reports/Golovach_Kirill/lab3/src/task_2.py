...
class ExpertLevel(AccountLevel):
    def handle_request(self, user_activity):
        if 50 <= user_activity < 100:
            print(
                "Вы находитесь на уровне Expert. Доступны скидки до 25% "
...
class VIPLevel(AccountLevel):
    def handle_request(self, user_activity):
        if user_activity >= 100:
            print(
                "Вы находитесь на уровне VIP. Доступны скидки до 50%, "
                "персональные предложения и приоритетная поддержка."
            )
        else:
...
class UserAccount:
    ...
    def add_purchase(self, book_title, book_cost):
        """Добавляет покупку книги и начисляет очки активности."""
        activity_points = int(book_cost)  # Очки активности равны стоимости книги
        self.user_activity += activity_points
        self.purchase_history.append((book_title, book_cost))
        print(f"Книга '{book_title}' стоимостью {book_cost} добавлена в историю покупок.")
    ...
    def choose_free_books(self):
        ...
        while len(self.free_books) < 3:
            user_choice = input(f"Выберите книгу (введите номер от 1 до {len(free_book_options)}): ")
            if user_choice.isdigit() and 1 <= int(user_choice) <= len(free_book_options):
                selected_book = free_book_options[int(user_choice) - 1]
    ...
    def show_purchase_history(self):
        ...
        print("\nИстория покупок:")
        for i, (book_title, book_cost) in enumerate(self.purchase_history, start=1):
            print(f"{i}. Книга: {book_title}, Стоимость: {book_cost}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == "1":
            ...
            book_cost = input("Введите стоимость книги: ")
            if book_cost.isdigit():
                user.add_purchase(book_title, int(book_cost))
            else:
