...
from PyQt5.QtWidgets import (
    ...
    QColorDialog, QDoubleSpinBox, QComboBox, QMessageBox
)
...
class MainWindow(QMainWindow):
    ...
    def create_depth_control(self, layout):
        ...
        depth_spin = QSpinBox()
        depth_spin.setRange(1, 15)
        depth_spin.setValue(8)
        depth_spin.valueChanged.connect(self.tree_widget.set_depth)
    ...
    def create_angle_control(self, layout):
        ...
        angle_spin = QSpinBox()
        angle_spin.setRange(0, 90)
        angle_spin.setValue(45)
        angle_spin.valueChanged.connect(self.tree_widget.set_angle)
    ...
    def create_wind_angle_control(self, layout):
        ...
        wind_angle_spin = QSpinBox()
        wind_angle_spin.setRange(0, 45)
        wind_angle_spin.setValue(15)
        wind_angle_spin.valueChanged.connect(self.tree_widget.set_wind_angle)
        wind_layout.addWidget(wind_angle_spin)
    ...
    def create_wind_dir_combo(self, layout):
        ...
        wind_dir_combo.addItems(["Слева", "Справа"])
        wind_dir_combo.currentIndexChanged.connect(self.tree_widget.set_wind_direction)
        dir_layout.addWidget(wind_dir_combo)
    ...
    def create_ratio_control(self, layout):
        ...
        ratio_spin.setValue(0.7)
        ratio_spin.valueChanged.connect(self.tree_widget.set_ratio)
        ratio_layout.addWidget(ratio_spin)
        layout.addLayout(ratio_layout)

    def create_color_buttons(self, layout):
        color1_button = QPushButton("Цвет ствола")
        color1_button.clicked.connect(lambda: self.choose_color(self.tree_widget.set_color1))
        layout.addWidget(color1_button)

        color2_button = QPushButton("Цвет листьев")
        color2_button.clicked.connect(lambda: self.choose_color(self.tree_widget.set_color2))
        layout.addWidget(color2_button)

        bg_color_button = QPushButton("Цвет фона")
        bg_color_button.clicked.connect(lambda: self.choose_color(self.tree_widget.set_bg_color))
        layout.addWidget(bg_color_button)

    def create_screenshot_button(self, layout):
        screenshot_btn = QPushButton("Сделать скриншот")
        screenshot_btn.clicked.connect(self.tree_widget.take_screenshot)
